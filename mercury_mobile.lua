...
-- ===============================
-- MERCURY MOBILE SUPPORT PATCH
-- Adds Open / Close / Toggle APIs without keyboard input
-- ===============================
do
    ...
    local function findMercuryGui()
        for _, v in ipairs(parent:GetChildren()) do
            if v:IsA("ScreenGui") and v.Name:lower():find("mercury") then
                return v
            end
        end
        return nil
    ...
    local function ensure()
        local gui = findMercuryGui()
        if not gui then
            -- wait briefly for Mercury to mount
            for _ = 1, 120 do
                task.wait(0.05)
                gui = findMercuryGui()
    ...
    getgenv().Mercury = getgenv().Mercury or {}

    function getgenv().Mercury.Open()
        local gui = ensure()
        if gui then gui.Enabled = true end
    end

    function getgenv().Mercury.Close()
        local gui = ensure()
        if gui then gui.Enabled = false end
    end

    function getgenv().Mercury.Toggle()
        local gui = ensure()
